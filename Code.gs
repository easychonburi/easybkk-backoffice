...
function bulkUpsert_(b){
  ...
  dateRange_(b.date_from,b.date_to).forEach(ds=>{
    if(skip.includes(ds)||existing.some(x=>x.staff_id===staff.staff_id&&x.date===ds)){skipped++;return}
...
function date_(d=new Date()){return Utilities.formatDate(d,TZ,'yyyy-MM-dd')}function time_(){return Utilities.formatDate(new Date(),TZ,'HH:mm')}function iso_(){return Utilities.formatDate(new Date(),TZ,'yyyy-MM-dd HH:mm:ss')}
function dateRange_(from,to){const out=[],start=new Date(String(from)+'T12:00:00Z'),end=new Date(String(to)+'T12:00:00Z');if(isNaN(start)||isNaN(end)||start>end)throw Error('ช่วงวันที่ไม่ถูกต้อง');for(let d=start;d<=end;d.setUTCDate(d.getUTCDate()+1))out.push(Utilities.formatDate(d,'UTC','yyyy-MM-dd'));return out}
