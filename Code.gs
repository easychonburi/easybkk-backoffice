...
const SHEETS = {
  staff: ['staff_id','name','nickname','pin','role','branch_id','daily_rate','ot_rate','status','created_at'],
  branches: ['branch_id','name','lat','lng','allowed_radius_m','status'],
  timesheets: ['record_id','staff_id','staff_name','branch_id','branch_name','date','clock_in','clock_out','hours_worked','late_min','ot_hours','ot_status','note','clock_in_lat','clock_in_lng','clock_out_lat','clock_out_lng','created_at'],
...
function setupSystem() {
  ...
  if (!sheetObjects_('staff').length) {
    ...
    append_('staff',{staff_id:'STF001',name:'ผู้ดูแลระบบ',nickname:'แอดมิน',pin,role:'admin',branch_id:'BR001',daily_rate:0,ot_rate:0,status:'active',created_at:iso_()});
  ...
  } else SpreadsheetApp.getUi().alert('ตรวจสอบโครงสร้างระบบเรียบร้อย');
}
...
function route_(b) {
  try {
    ...
    if (b.action === 'login') data = login_(b.pin);
      ...
      const adminOnly = ['adminData','saveStaff','saveBranch','saveSettings','saveTelegram','bulkUpsertTimesheets','saveLeave','saveAdvance','approveOT','payrollPreview','finalizePayroll'];
...
function clockIn_(user,b) {
  ...
  const settings=settings_(), now=time_(), late=Math.max(0,minutes_(now)-minutes_(settings.shift_start));
  const row={record_id:id_('TS'),staff_id:user.staff_id,staff_name:user.nickname||user.name,branch_id:branch.branch_id,branch_name:branch.name,date:today,clock_in:now,clock_out:'',hours_worked:'',late_min:late,ot_hours:0,ot_status:'none',note:'',clock_in_lat:b.lat,clock_in_lng:b.lng,created_at:iso_()};
  append_('timesheets',row); telegram_(`✅ ${row.staff_name} เข้างาน ${now}\n📍 ${branch.name}${late>Number(settings.late_grace_min||0)?`\n⚠️ สาย ${late} นาที`:''}`); return row;
}
function clockOut_(user,b) {
  ...
  const branch=nearestBranch_(Number(b.lat),Number(b.lng)), now=time_(), settings=settings_();
  const hours=hours_(row.clock_in,now),otMin=overtimeMinutes_(row.clock_in,settings.shift_end,now);
  const otHours=otMin>Number(settings.ot_grace_min||0)?Number((otMin/60).toFixed(2)):0;
...
function myDashboard_(user) {
  ...
  return {today:rows.find(x=>x.date===date_())||null,history:rows.slice(0,7)};
}

function adminData_() {
  const staff=sheetObjects_('staff'),branches=sheetObjects_('branches'),times=sheetObjects_('timesheets'),today=date_();
  const withBranch=staff.map(s=>({...s,branch_name:(branches.find(b=>b.branch_id===s.branch_id)||{}).name||''}));
  const ot=times.filter(x=>x.ot_status==='pending');
  return {staff:withBranch,branches,settings:settings_(),overview:{active_staff:staff.filter(x=>x.status==='active'&&x.role!=='admin').length,present_today:times.filter(x=>x.date===today&&x.clock_in).length,pending_ot:ot.length,today:times.filter(x=>x.date===today),ot_items:ot}};
}
function saveStaff_(b) {
  ...
  const data={name:b.name,nickname:b.nickname,pin:String(b.pin),role:b.role||'staff',branch_id:b.branch_id,daily_rate:Number(b.daily_rate||0),ot_rate:Number(b.ot_rate||0),status:b.status||'active'};
...
function bulkUpsert_(b){
  ...
  for(let d=new Date(b.date_from+'T00:00:00');d<=new Date(b.date_to+'T00:00:00');d.setDate(d.getDate()+1)){
    ...
    append_('timesheets',{record_id:id_('TS'),staff_id:staff.staff_id,staff_name:staff.nickname||staff.name,branch_id:staff.branch_id,branch_name:branch.name||'',date:ds,clock_in:b.clock_in,clock_out:b.clock_out,hours_worked:hours_(b.clock_in,b.clock_out),late_min:Math.max(0,minutes_(b.clock_in)-minutes_(settings_().shift_start)),ot_hours:0,ot_status:'none',note:'เพิ่มย้อนหลังโดยแอดมิน: '+b.admin_note,created_at:iso_()});created++;
...
function payrollPreview_(month){
  if(!/^\d{4}-\d{2}$/.test(month||''))throw Error('กรุณาเลือกเดือน');const settings=settings_(),shiftMinutes=Math.max(1,minutesOvernight_(settings.shift_start,settings.shift_end));
  ...
  return staff.map(s=>{const t=times.filter(x=>x.staff_id===s.staff_id),days=new Set(t.filter(x=>x.clock_in).map(x=>x.date)).size,paidLeave=countPaidLeaveDays_(leaves.filter(x=>x.staff_id===s.staff_id&&x.status==='approved'),month),rate=Number(s.daily_rate||0),otHours=t.filter(x=>x.ot_status==='approved').reduce((a,x)=>a+Number(x.ot_hours||0),0),late=t.reduce((a,x)=>a+Math.max(0,Number(x.late_min||0)-Number(settings.late_grace_min||0)),0),lateDeduct=settings.late_deduct_mode==='none'?0:rate/shiftMinutes*late,advance=adv.filter(x=>x.staff_id===s.staff_id).reduce((a,x)=>a+Number(x.amount||0),0),base=rate*(days+paidLeave),ot=otHours*Number(s.ot_rate||0);return{staff_id:s.staff_id,staff_name:s.nickname||s.name,days_worked:days,paid_leave_days:paidLeave,base_pay:round_(base),ot_pay:round_(ot),late_deduct:round_(lateDeduct),advance_deduct:round_(advance),manual_adjust:0,total_pay:round_(base+ot-lateDeduct-advance)}});
}
...
function ensureSheet_(name,headers){const ss=SpreadsheetApp.getActiveSpreadsheet();let s=ss.getSheetByName(name);if(!s)s=ss.insertSheet(name);if(s.getLastRow()===0)s.getRange(1,1,1,headers.length).setValues([headers]).setFontWeight('bold').setBackground('#dceff7');return s}
...
function minutes_(t){const [h,m]=String(t).split(':').map(Number);return h*60+m}function minutesOvernight_(start,end){let a=minutes_(start),b=minutes_(end);if(b<a)b+=1440;return b-a}function hours_(a,b){return Number((minutesOvernight_(a,b)/60).toFixed(2))}function round_(n){return Math.round((Number(n)||0)*100)/100}
